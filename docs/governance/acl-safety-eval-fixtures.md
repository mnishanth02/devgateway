...
| Semantic cache pre-enablement leak prevention | `acl-safety-semantic-cache-preenablement-deny` | Deny semantic cache lookup/write and cache-existence probing until scoped-cache enablement gates are complete. | trace, principal, project, decision, ACL scope, enablement state, cache key, denial reason. |
...
Run `pnpm workspace:validate` to validate schema catalog integrity. Inspect the dataset categories with a JSON check to ensure all required categories and semantic-cache pre-enablement coverage are present before wiring any eval runner.
