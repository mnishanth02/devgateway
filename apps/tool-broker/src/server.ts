...
export function createRequestHandler(port: number) {
    return (request: IncomingMessage, response: ServerResponse) => {
        ...
        writeJson(response, 404, {
            error: 'not_found',
            message: 'DevGateway Tool Broker local skeleton exposes /healthz and /readyz.',
        });
    };
}

export function startServer(port = parsePort(process.env.PORT, 43102)) {
    const server = createServer(createRequestHandler(port));
    server.listen(port, '127.0.0.1', () => {
        console.log(`@devgateway/tool-broker listening at http://127.0.0.1:${port}`);
    });
    return server;
}
