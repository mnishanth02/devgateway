...
ALTER TABLE "step_attempt" ADD CONSTRAINT "step_attempt_replay_decision_check" CHECK ("step_attempt"."replay_decision" IS NULL OR "step_attempt"."replay_decision" IN ('replay', 'skip', 'abort', 'pre_side_effect', 'idempotent_pre_side_effect', 'ambiguous_post_side_effect', 'retry_scheduled', 'terminal_failure', 'manual_review'));--> statement-breakpoint
