...
| Bifrost/provider | Gateway owner | `BIFROST_ADMIN_TOKEN`, `BIFROST_VIRTUAL_KEY_SEED`, provider API keys | `BIFROST_BASE_URL`, config/policy versions, break-glass flag | Generated or validated config from registry snapshot; stale/missing policy fails closed. |
...
- Non-secret version variables such as `BIFROST_ROUTE_CONFIG_VERSION` and `BIFROST_POLICY_VERSION` rotate with approved registry/policy snapshots and must be validated before startup.
