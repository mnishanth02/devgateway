...
|---|---|---|---|---|---|---|
| Bifrost gateway | Platform architecture owner | Platform operations owner | `docker build -f infra\bifrost\Dockerfile .` or approved upstream image | Upstream Bifrost entrypoint with `BIFROST_CONFIG_PATH` | `infra\bifrost\**`, `packages\registry\**`, `packages\policy\**`, `packages\observability\**` | `/health` or upstream equivalent; fail closed if registry/policy config cannot load. |
