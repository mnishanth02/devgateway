...
| Metrics | RED metrics, queue depth, lease age, durable worker heartbeat/profile counters, provider latency/errors, retrieval latency, gate pass/fail counts, cost counters, backup status, SLO burn rates. | Prometheus and Grafana. | Platform operations owner |
...
| Workflow / workers | Queue depth, lease age, running/failed/cancelled workflows, runtime-service claims/completions/failures, lease/retry recoveries/escalations, cancellation propagation, approval expiry count/oldest pending age, outbox delivered/retried/dead-lettered, budget reservations released, artifact lifecycle reconciliation, step latency, artifact write errors, audit write failures. | Platform operations owner | Platform architecture owner |
...
5. Confirm durable worker heartbeat/backlog panels show local or fixture data only when workers are explicitly started.
6. Confirm fixture eval runs emit gate metrics and artifact links.
...
2. Run a workflow fixture and verify queue/lease and durable worker profile panels update.
...
| Missing durable worker heartbeat/profile metrics | Worker profile not explicitly launched, missing `OPERATIONAL_DATABASE_URL`, stale OTLP config, failed profile readiness, or production gate disabled the service as expected. | Platform operations owner |
